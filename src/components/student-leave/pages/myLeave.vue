...
export default {
  ...
  methods: {
    ...
    onAction (item, i) {
      this.item = item
      this.$router.push({ name: 'detailMyLeave', params: { id: item.ask_id } })
    },
    getData (page, k = '') {
      let reg = /^\d+$ /
      ...
      return this.$axios.get(`/palm/v2/userask?condition=append&view=${view}&page=${page}&size=10&${type}=${k}`).then(({ data }) => {
