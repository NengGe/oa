...
export default {
  ...
  methods: {
    ...
    //     setTimeout(() => {
    //       this.classData[this.dormId] = d
    //       // this.$set(this.classData, this.dormId, d)
    //       resolve(d)
    //     }, 100)
    //   })
    // },
    getStudentList () {
      ...
      return this.$axios.get(`/palm/v2/dorm/user/list/${this.dormId}`).then(({ data }) => {
        this.$preloaderFull.close()
        if (data.status === 200) {
          let d = data.result.data.map(item => {
