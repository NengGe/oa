...
export default {
  ...
  methods: {
    onreject () {
      this.postApprove('reject').then(d => {
        d === 200 && this.$simpleMsg('已驳回')
      })
    },
    ondelete () {
      this.postDel(this.eventId).then(d => {
        d === 200 && this.$simpleMsg('已删除')
      })
    },
    onedit () {
      this.$router.push({ name: 'behaviroEntryIndex', params: { type: '4' }, query: { eventId: this.eventId } })
    },
    getData () {
    ...
    postDel (id) {
      this.$preloaderFull.show()
      return this.$axios.get(`/palm/v2/ruledel/${id}`).then(({ data }) => {
        this.$preloaderFull.close()
        if (data.status === 200) {
          this.$router.back()
        } else {
