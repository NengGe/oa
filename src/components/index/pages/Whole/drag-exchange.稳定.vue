...
export default {
      ...
      if (this.dragArr.length > newV.length) { // 执行了删除
        ...
        this.$nextTick(() => {
          ...
          for (let i = this.dragArr.length; i < newV.length; i++) {
            ...
            // console.log('this.data[i].display_order', this.data[i].display_order)
  ...
  created () {
    this.initGridSite()
    // console.log('this.gridSiteArr', this.gridSiteArr)
  },
  methods: {
    changeOrder () {
    ...
    onMove (e, that) {
      ...
      // if (this.lastDrag && this.lastDrag.order !== i) {
      //   this.lastDrag.scrollTo(this.gridSiteArr[this.lastDrag.order], 300)
      //   this.lastDrag = null
      // }
      // if (i !== -1 && i !== that.order) { // 进入模板区域 && 不是当前元素本身区域
      //   // console.log('***********')
      ...
      //   })
      //   if (dragIsExchanged) {
      //     dragIsExchanged.scrollTo(this.gridSiteArr[that.order], 300)
    ...
    toMove (start, end, flag) {
      this.dragArr.forEach((drag, i) => {
        if (drag.order >= start && drag.order <= end) {
          let order = parseInt(drag.order) + flag // 数字字符 +1 or -1
