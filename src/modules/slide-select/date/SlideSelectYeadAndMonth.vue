  ...
  export default {
    ...
    created () {
      if (this.value) {
        ...
        let listDataArr = dateBaseHandle.getData(this.startYear, currData[0], currData[1])
        this.lists = [listDataArr[0], listDataArr[1]]
        // this.lists[2].i = currData[2] - 1
      } else {
        let listDataArr = dateBaseHandle.getData(this.startYear, todayYear, todayMonth)
        this.lists = [listDataArr[0], listDataArr[1]]
        this.selectTody()
      }
    },
    methods: {
      ...
      onConfirm (close) {
        ...
        this.$emit('update:value', `${d1.d[d1.i]}-${d2.d[d2.i]}`)
        close(1)
        this.$emit('curPageConfirm', `${d1.d[d1.i]}-${d2.d[d2.i]}`)
