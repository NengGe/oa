    ...
    <div class="select-box" :class="{active: isShowSelectBox}">
      ...
      <div class="select-month">
        <ul class="monthList cf">
          <li class="month" :class="{active: parseInt(currMonth)===i+1}" @click="monthActive(i)" v-for="(month, i) in monthList" :key="'monthList'+i">{{month}}
          </li>
        </ul>
      </div>
    </div>
      ...
      <div class="swiper-container">
        <div class="wrapper cf" ref="wrapper" :class="{active: isActiveWrapper}">
          ...
          <div class="day-box last">
            <ul class="dayList  cf">
              <li class="day" :class="{'noCurrWeek': parseInt(oDate.month)%12!==parseInt(currMonth - 1)}" v-for="(oDate, i) in lastMonthDayList" :key="'lastMonthDayList'+i">
          ...
          <div class="day-box curr">
            <ul class="dayList  cf">
              <li @click="dayAction(oDate)" class="day" :class="{'noCurrWeek': parseInt(oDate.month)!==parseInt(currMonth)}" v-for="(oDate, i) in dayList" :key="'dayList'+i">
          ...
          <div class="day-box next">
            <ul class="dayList  cf">
              <li class="day" :class="{'noCurrWeek': parseInt(oDate.month)!==parseInt(currMonth)%12+1}" v-for="(oDate, i) in nextMonthDayList" :key="'nextMonthDayList'+i">
                <span class="num">{{oDate.day}}</span>
              </li>
            </ul>
          </div>
...
export default {
  ...
  watch: {
    'dateStr': function () {
      ...
      this.getAllDayList(this.currYear, this.currMonth, this.currDay)
  ...
  methods: {
    ...
    dayAction (oDate) {
      this.currYear = oDate.year
      this.currMonth = oDate.month
      this.currDay = oDate.day

      // if (parseInt(day.month) === parseInt(this.currMonth)) {
      //   this.currDay = day.day
      // }
    },
    getAllDayList (currYear, currMonth, currDay) {
      console.log(currYear, currMonth, currDay)
      this.dayList = this.getDayList(currYear, currMonth, currDay)
      this.lastMonthDayList = this.getDayList(currYear, currMonth, currDay - 7)
      this.nextMonthDayList = this.getDayList(currYear, currMonth, currDay - 0 + 7)
    },
    getDayList (currYear, currMonth, currDay) {
      // console.log(currYear, currMonth, currDay)
      let d = new Date(currYear, currMonth - 1, currDay)
      let dayOfWeek = d.getDay() // 1号星期几
      let dayList = []
      for (let i = 0; i < 7; i++) {
        let oDate = new Date(currYear, currMonth - 1, currDay - dayOfWeek + i)
        dayList.push({
    ...
    weekSubAdd (p) {
      let d = new Date(this.currYear, parseInt(this.currMonth) - 1, this.currDay + parseInt(p) * 7)
      this.getCurrDate(d)
    },
    yearSubAdd (p) {
      this.currYear += p
