...
export default class {
  init ({elem, elemz}) {
    ...
    let {clientWidth: elw, clientHeight: elh} = elem // 放大元素高宽，也是容器高宽，初始高宽
    let {offsetWidth: imgw, offsetHeight: imgh} = elemz // 放大元素高宽，也是容器高宽，初始高宽
    if (imgw === 0 || imgh === 0) {
    ...
    function toScale (x, y) {
      // transform-origin 默认是50%. 这样写可能更容易明白： imgw * 0.5 * (scale - 1)
      let otherX = imgw / 2 * (currScale - 1)
      let otherY = imgh / 2 * (currScale - 1)
      x = x + otherX
      y = y + otherY
    ...
    drag({
      ...
      onStart: (cPageX, cPageY) => {
        ...
        let w = imgw * currScale
        let h = imgh * currScale
