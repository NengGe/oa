  ...
  buildWeekData (date) {
    ...
    function weekHalfHandle (d, w) {
      ...
      if (v < 0) { // 上月
        ...
        endDate = currYear + '-' + numfill(currMonth)
      ...
      if (firstDayNum) {
        ...
        if (endDate) {
          ...
          dayKeyValHalf = nextDate.getFullYear() + '-' + numfill(nextDate.getMonth() + 1) + '-'
        }
        for (let i = 1, len = 8 - firstDayNum; i < len; i++) {
          daysKey[i] = dayKeyValHalf + numfill(i)
          secondDays.push(i)
        }
        endDate = dayKeyValHalf + numfill(secondDays[secondDays.length - 1])
      } else {
